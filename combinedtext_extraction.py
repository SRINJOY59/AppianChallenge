...
class CombinedDocumentParser:
    ...
    def parse_document(self, file_path: str, output_dir: str = "parsed_output") -> Tuple[str, str]:
        """Parse document using both parsers and save results."""
        ...
        try:
            ...
            if file_path.lower().endswith('.pdf'):
                ...
                llama_text = self.parse_with_llama(file_path)
                llama_output = output_path / f"{file_name}_llama_{timestamp}.txt"
                with open(llama_output, 'w', encoding='utf-8') as f:
                    f.write(llama_text)
            ...
            return str(groundx_output), str(llama_output)
...
def main():
    ...
    try:
        ...
        # # #
        # # # #
        input_file = "text_extraction/Driving_License.pdf"  # Replace with your PDF file
        # # # #
        # # #
        # #
        #
        
        if(parser.is_scanned_pdf(input_file)):
            print("scanned pdf")
            groundx_output, llama_output = parser.parse_document(
                file_path=input_file,
                output_dir="parsed_output"
            )
            
            logger.info("Document processing completed successfully")
            logger.info(f"GroundX results: {groundx_output}")
            logger.info(f"LlamaParse results: {llama_output}")
        else:
            print("machine readable pdf")
            extracted_content = parser.process_pdf(input_file)
            print("Extracted Text:")
            print(extracted_content["text"])
